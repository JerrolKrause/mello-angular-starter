...
//Main service to handle logging and analytics 
@Injectable()
export class LoggingService {
    ...
    prodHost: string = "agent.loandepot.com"   //Prod domain, used to switch tokens. This should match the hostname
    tokenCurrent: string = "12b6209da24f589a1e85535df6db3bf3"   //Mixpanel dev token
    tokenProd: string = "eea5001f0d24f84c3ae2b6ccfef2193f"  //Mixpanel prod token
    gaToken: string = "UA-12105805-4" // GA prod token, no dev token
    eventQueue: any[] = [] //Hold and queue events if mixpanel hasn't been loaded yet. This manages out of order/async events
    isLoaded: boolean = false //Wait for the analytics scripts to be loaded and register all the initial properties before the first event is passed
    ...
    */
    private init() {
        ...
        //Register loan number if present
        if (window.loanGuid) {
            mixpanel.register({ "Loan Number": window.loanGuid });
        }

        //Now that the first event has been loaded, set the loaded flag to true
        this.isLoaded = true;

        //First event passed to mixpanel
        this.trackEvent("Agent Logged In");
